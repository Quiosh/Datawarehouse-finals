...
def main(new_campaign_file: bytes = None):
    ...
    if new_campaign_file:
        ...
        # Fallback to URL
        print(f"🌐 No upload provided. Attempting to load from URL: {URL_LATE_CAMPAIGN_FILE}")
        try:
            df_new_campaigns = _standardize_campaign_df(_load_csv_from_url(URL_LATE_CAMPAIGN_FILE))
